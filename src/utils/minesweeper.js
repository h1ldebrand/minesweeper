...
export function generateBoard({cols, rows, mines}){
    const cell = Map({isReveal: false, isFlagged: false})
...
export function getAdjacentTiles(game, tileId){
    return getAdjacentTileIds(game, tileId)
        .map(id => game.getIn(['board', id])).filter(tile => tile.get('isMine')).size
...
export function revealTile(game, tileId) {
    ...
    return updateGame.getIn(['board', tileId, 'isMine'])
        ? revealAllMines(updateGame)
        : revealAdjacentSafeTiles(updateGame, tileId);
}
